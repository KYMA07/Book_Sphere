...
function Dashboard() {
  ...
  const pendingAppointments = (Array.isArray(appointments) ? appointments : []).filter(
    (a) => a.status === 'pending'
  ).length;
      ...
      {selectedRecord && (
        <div className="modal">
          <div className="modal-content">
            ...
            <p>
              <strong>Student:</strong> {selectedRecord.Student?.full_name} (
              {selectedRecord.Student?.student_number})
            </p>
            <p>
              <strong>Book:</strong> {selectedRecord.Book?.title}
            </p>
            <p>
              <strong>Librarian:</strong> {selectedRecord.User?.username}
            </p>
            <p>
              <strong>Borrow Date:</strong>{' '}
              {selectedRecord.borrow_date
                ? new Date(selectedRecord.borrow_date).toLocaleString()
                : '-'}
            </p>
            <p>
              <strong>Due Date:</strong>{' '}
              {selectedRecord.due_date
                ? new Date(selectedRecord.due_date).toLocaleString()
                : '-'}
            </p>
            <p>
            ...
            {selectedRecord.return_date && (
              <p>
                ...
                {new Date(selectedRecord.return_date).toLocaleString()}
              </p>
            )}
